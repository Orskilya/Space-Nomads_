                            ...
                            for i in range(self.shop_buttons[row]['amount']):
                                if self.shop_buttons[row]['x'][0] + i * self.shop_buttons[row][
                                    ...
                                    if str(item) in 'photon absorber destructor':
                                        item.set_bullet_img(load_image(str(item) + '_bullet.png', item.get_size(), -1))
...
hero = Hero.Hero(
    Ships.NomadShip(load_image('Nomad_ship_fly.png', (64, 64)), [WIDTH // 2, HEIGHT // 2],
                    500, 0, [Equipments.Destructor(3, (enemy, all_sprites),
                                                   load_image('destructor_bullet.png', (50, 40), -1)),
                             Equipments.Engine(3), ],
                    camera,
                    SIZE, all_sprites, ships, hero_group), 10000, None)
